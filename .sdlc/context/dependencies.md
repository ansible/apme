...
| Package | Purpose |
|---------|---------|
| `abbenay-client==2026.8.9` | gRPC client for the Abbenay AI provider (Tier 2 remediation); PyPI, CalVer-aligned with daemon image `v2026.8.9`. Local/dev: `uv sync --extra ai`. Production/containers: `uv sync --frozen --extra ai` (hashes in `uv.lock`). `pip install apme-engine[ai]` is version-pin only. |
