...
These rules are generated by the deprecation pipeline (`scripts/generate_deprecation_rules.py`) and detect patterns deprecated in upcoming ansible-core releases.
...
| M015 | native | `play_hosts` magic variable deprecated; use `ansible_play_hosts` | — |
| M016 | OPA | Empty `when:` on task is deprecated and will error in 2.23 | — |
| M017 | OPA | `action` key mapping in `tasks_from` deprecated; use FQCN module names | — |
| M018 | OPA | Paramiko connection plugin deprecated; use `ansible.builtin.ssh` | — |
| M019 | native | `!!omap` / `!!pairs` YAML tags deprecated | — |
| M020 | native | `!vault-encrypted` YAML tag deprecated | — |
| M021 | OPA | Empty `args:` / `vars:` on task deprecated | — |
...
| M024 | OPA | `include_vars` `ignore_files` deprecated; use `ignoreerrors` param | — |
