...
- **Localhost**: Engine → Abbenay Unix socket (`unix:///tmp/abbenay-run/abbenay/daemon.sock`; required when a consumer token is set); reporting → `127.0.0.1:50060`. Abbenay still binds gRPC on `127.0.0.1:50057` for in-container health probes (no TLS for the chart path).
