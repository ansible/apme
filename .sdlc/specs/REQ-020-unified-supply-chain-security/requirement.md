...
- **THEN** suppressed advisories appear in `vulnerabilities[]` with CycloneDX
  `analysis.state` mapped from `vex_state`, `analysis.justification` mapped from
  `vex_justification` (when present), and `analysis.detail` containing the concatenated
...
- **AND** `sbom_url` references the full enriched CycloneDX export for the summarized
  scan: `/api/v1/projects/{project_id}/sbom?scan_id={scan_id}&include=vulnerabilities,vex`
  (inventory-only default without `include` remains unchanged per AC-8)
