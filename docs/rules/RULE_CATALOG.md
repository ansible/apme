...
**157 rules** across 4 validators — the single reference for every rule.
...
| Severity resolved (ADR-043 table or SEC: prefix) | 156/157 |
| Severity fallback (medium default) | 1/157 |
...
| M031 | M | Native | medium | task | ai | Variables with sensitive names should use Sensitive tag (ansible-core 2.19+). | Yes | Yes | Yes | — |
...
| M031 | medium | task | ai | Variables with sensitive names should use Sensitive tag (ansible-core 2.19+). | Yes | Yes | Yes | — |
...
### Missing from graph/severity.py (fallback to medium) — 1

- **M031** (Native): add to `SEVERITY_DEFAULTS` per ADR-043
