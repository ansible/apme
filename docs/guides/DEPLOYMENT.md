...
- NetworkPolicy for Ingress → Gateway/UI HTTP ports when enabled in the operator
- PVCs for sessions, PostgreSQL (`postgres-data`), and Galaxy Proxy cache
- PostgreSQL included in the reference topology; external PostgreSQL via operator CR /
  `APME_DATABASE_URL` with certificate-validated TLS for remote hosts
...
environments. Older release tags remain amd64-only until rebuilt; nodes pull
the matching platform from multi-arch manifests. Prefer a post-ADR-063 SHA or
release tag (and Quay only when that publish included Quay credentials).
